...
public class ChessMatch {
	...
	private ChessPiece king(Color color) {
		...
		// Se não encontrar o rei, você pode lançar uma exceção ou retornar null,
		// dependendo da lógica do seu jogo.
	...
	private boolean testCheckMate(Color color) {
			...
			for (int i = 0; i < board.getRows(); i++) {
				for (int j = 0; j < board.getColumns(); j++) {
					if (mat[i][j]) {
						...
						if (!testCheck) {
							return false; // Existe pelo menos um movimento que resolve o xeque.
						}
					}
				}
			}
		...
		return true; // Nenhum movimento resolve o xeque, é xeque-mate.
	}
